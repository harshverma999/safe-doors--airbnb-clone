...
module.exports.isLoggedIn = (req, res, next) => {
    if (!req.isAuthenticated()) {
        ...
        let message = "You must be logged in to continue.";

        if (req.method === "DELETE" && req.originalUrl.includes("/reviews/")) {
            message = "You must be logged in to delete a review.";
        } else if (req.method === "POST" && req.originalUrl.includes("/reviews")) {
            message = "You must be logged in to add a review.";
        } else if (req.method === "POST" && req.originalUrl === "/listings") {
            message = "You must be logged in to create a new listing.";
        }
